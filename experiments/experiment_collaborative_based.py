...
import django
django.setup()
# Import the collaborative-based model and helper functions
from core.recommenders.collaborative_based import recommend_movies_from_csv
...
if __name__ == "__main__":
    ...
    for user_id in tqdm(user_ids, desc="Evaluating users"):
        user_results = evaluate_user_at_ks(user_id, ratings_df, k_values, recommender)
        all_results.extend(user_results)
    # Turn all results into a DataFrame
    results_df = pd.DataFrame(all_results)
    # Save the results to a CSV file
    results_df.to_csv("collaborative_eval_results.csv", index=False)
    print("Results saved to collaborative_eval_results.csv")
    # Plot the metrics in a line chart
    plot_precision_recall_ndcg(
        results_df, 
        title="Collaborative Filtering Recommender Performance", 
        filename="collaborative_precision_recall_ndcg.png"
    )
    ...
    summary_path = "collaborative_eval_summary.csv"
